...
const ratingCategories = [
  {
    ...
    emoji: "💰",
    description: "Overall affordability",
  },
  {
    id: "safety",
    label: "Safety",
  ...
  },
  {
    ...
    emoji: "🚗",
    description: "Public transport quality",
  },
  {
    id: "healthcare",
    label: "Healthcare",
    emoji: "🏥",
    description: "Medical care quality",
  },
  {
    id: "food",
    label: "Food Quality",
    emoji: "🍽️",
    description: "Local cuisine and food safety",
  },
  {
    id: "nightlife",
    label: "Nightlife",
    emoji: "🍸",
    description: "Night entertainment options",
  },
  {
    id: "culture",
    label: "Culture",
    emoji: "🎭",
    description: "Cultural attractions and events",
  },
  {
    id: "outdoorActivities",
    label: "Outdoor Activities",
    emoji: "🌳",
    description: "Parks, hiking, outdoor fun",
  },
];

const ratingLabels = ["Bad", "Okay", "Good", "Great", "Amazing"];

function ComprehensiveRatingFormInner({
  cityName = "Colombo",
  selectedCity,
  user,
}) {
  ...
  const actualUserId = user?.sub || "guest-user";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8080/api/cities/${selectedCity.cityId}/ratings`,
        {
          method: "POST",
                ...
                <div className="flex gap-2 ml-12">
                  {ratingLabels.map((label, index) => (
                    <button
                      ...
                      className={`px-4 py-2 text-sm font-medium rounded-lg border transition-colors ${ratings[category.id] === index + 1 ? "bg-primary text-primary-foreground border-primary" : "bg-background hover:bg-muted border-border"}`}
                    >
                      {label}
                    </button>
                  ))}
...
export default function ComprehensiveRatingForm(props) {
  return (
    <User>
      {(user) => <ComprehensiveRatingFormInner {...props} user={user} />}
    </User>
  );
}
