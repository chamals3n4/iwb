    ...
    const fetchCity = async () => {
      try {
        const res = await fetch(
          `http://localhost:8080/api/cities/${params.slug}`,
          { cache: "no-store" },
        );
      ...
      <div className="container mx-auto px-8 py-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid w-full grid-cols-3">
            ...
            <TabsTrigger value="rate">Rate City</TabsTrigger>
          ...
          <TabsContent value="rate">
            <ComprehensiveRatingForm cityName={city.name} selectedCity={city} />
          </TabsContent>

          <TabsContent value="chat">
            <ChatInterface cityId={city.cityId} />
          </TabsContent>
        </Tabs>
      </div>
    </div>
